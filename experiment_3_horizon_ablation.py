"""
Experiment 3: Forecast Horizon Ablation (PatchTST Patchwise Only)

Tests forecast horizons [24, 96, 336] hours on PatchTST Patchwise model only.
Fixed: SEQ_LEN from Exp 2, PATCH_LEN from Exp 1
Runs: 3 horizon configs x 1 model x 3 seeds = 9 total
"""
...
class Config:
    """Configuration for forecast horizon ablation."""
    FEATURES = ["temperature", "wind_speed", "humidity", "pressure"]
    SEQ_LEN = 672  # Should be updated from Experiment 2 results
    PRED_LEN = 96  # Will be varied in this experiment
...
def run_single_horizon_config(config, pred_len):
    """Run PatchTST Patchwise for one forecast horizon configuration."""
    config.PRED_LEN = pred_len
    config.CHECKPOINT_DIR = f"./checkpoints/experiment3_horizon/horizon_{pred_len}"
    config.RESULTS_DIR = f"./results/experiment3_horizon/horizon_{pred_len}"
...
def plot_horizon_ablation(all_horizon_results, pred_lengths):
    """Generate combined plot with overall and per-station results."""
    summary_dir = Path("./results/experiment3_horizon/summary")
    ...
    # Get station names
    stations = set()
    for horizon_results in all_horizon_results.values():
        if 'per_station' in horizon_results[model]:
            stations.update(horizon_results[model]['per_station'].keys())
    ...
    overall_mse_means = [all_horizon_results[p][model]['mse_mean'] for p in pred_lengths]
    overall_mse_stds = [all_horizon_results[p][model]['mse_std'] for p in pred_lengths]

    ax1.plot(pred_lengths, overall_mse_means, marker='o', linewidth=2.5, linestyle='-',
             label='Overall MSE (solid)', color=colors['overall'])
    ax1.fill_between(pred_lengths,
    ...
    for station in stations:
        ...
        for p in pred_lengths:
            if station in all_horizon_results[p][model].get('per_station', {}):
                means.append(all_horizon_results[p][model]['per_station'][station]['mse_mean'])
                stds.append(all_horizon_results[p][model]['per_station'][station]['mse_std'])

        if means:
            ax1.plot(pred_lengths, means, marker='o', linewidth=2, linestyle='-',
                     label=f'{station.capitalize()} MSE (solid)', color=colors.get(station, '#95a5a6'), alpha=0.7)

    # Plot overall MAE (dashed line)
    overall_mae_means = [all_horizon_results[p][model]['mae_mean'] for p in pred_lengths]

    ax2.plot(pred_lengths, overall_mae_means, marker='s', linewidth=2.5, linestyle='--',
    ...
    for station in stations:
        ...
        for p in pred_lengths:
            if station in all_horizon_results[p][model].get('per_station', {}):
                mae_means.append(all_horizon_results[p][model]['per_station'][station]['mae_mean'])

        if mae_means:
            ax2.plot(pred_lengths, mae_means, marker='s', linewidth=2, linestyle='--',
    ...
    ax1.set_xticks(pred_lengths)

    ax2.set_ylabel('MAE (Mean)', fontweight='bold', fontsize=12, color='gray')
    ax2.tick_params(axis='y', labelcolor='gray')

    ax1.set_title('Forecast Horizon Ablation: PatchTST Patchwise',
    ...
    plt.tight_layout()
    plt.savefig(summary_dir / 'horizon_ablation_combined.pdf', bbox_inches='tight', dpi=300)
    plt.close()
...
def run_horizon_ablation():
    """Main function for forecast horizon ablation study."""
    config = Config()

    print("=" * 70)
    print("EXPERIMENT 3: FORECAST HORIZON ABLATION (PATCHTST PATCHWISE)")
    print("=" * 70)
    print("Testing horizons: [24, 96, 336] hours (1, 4, 14 days)")
    print("Fixed: SEQ_LEN from Exp 2, PATCH_LEN from Exp 1")
    ...
    for pred_len in HORIZONS:
        ...
        results = run_single_horizon_config(config, pred_len)
        all_horizon_results[pred_len] = results

        print(f"\nHorizon {pred_len} complete")
        model = 'patchtst_patchwise'
    ...
    with open(summary_dir / 'summary.txt', 'w') as f:
        f.write("=" * 90 + "\n")
        f.write("FORECAST HORIZON ABLATION - PATCHTST PATCHWISE ONLY\n")
        f.write("=" * 90 + "\n")
        f.write(f"Tested: PRED_LEN = {HORIZONS} hours\n")
        f.write(f"Fixed: SEQ_LEN from Exp 2, PATCH_LEN from Exp 1\n")
        f.write(f"Runs per config: 3 (seeds: [42, 123, 456])\n")
        f.write("=" * 90 + "\n\n")

        f.write("OVERALL RESULTS (Combined performance across both stations)\n")
        f.write("-" * 90 + "\n")
        f.write(f"{'Horizon(h)':<12} {'MSE (mean±std)':<25} {'MAE (mean±std)':<25}\n")
        f.write("-" * 90 + "\n")

        for p in HORIZONS:
            res = all_horizon_results[p][model]
        ...
        for p in HORIZONS:
            all_stations = set()
            if 'per_station' in all_horizon_results[p][model]:
                all_stations.update(all_horizon_results[p][model]['per_station'].keys())

            if all_stations:
                f.write(f"\nHorizon {p}h:\n")
                for station in sorted(all_stations):
                    if station in all_horizon_results[p][model].get('per_station', {}):
                        res = all_horizon_results[p][model]['per_station'][station]
                        f.write(f"  {station.upper()}: MSE: {res['mse_mean']:.4f}±{res['mse_std']:.4f}  ")
                        f.write(f"MAE: {res['mae_mean']:.4f}±{res['mae_std']:.4f}\n")

        best_horizon = min(HORIZONS, key=lambda p: all_horizon_results[p][model]['mse_mean'])
        best_mse = all_horizon_results[best_horizon][model]['mse_mean']
        f.write("\n" + "=" * 90 + "\n")
        f.write(f"BEST FORECAST HORIZON: {best_horizon}h (Based on lowest overall MSE: {best_mse:.4f})\n")
        f.write("=" * 90 + "\n")
    ...
    best_horizon = min(HORIZONS, key=lambda p: all_horizon_results[p][model]['mse_mean'])
    print(f"\nBest horizon: {best_horizon}h (MSE: {all_horizon_results[best_horizon][model]['mse_mean']:.4f})")

    plot_horizon_ablation(all_horizon_results, HORIZONS)

    print(f"\nResults saved to {summary_dir}/")
    print(f"Optimal PRED_LEN: {best_horizon}")
